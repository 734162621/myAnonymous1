...
class MainAgentLLMPysc2(MainAgent):
  def __init__(self):
    ...
    api_key = 'sk-TNnwNgOQwic0J9TziiYu9wmpnml4cYt3OI4SfAKCmLV8zHF4'
  ...
  def step(self, obs):
    ...
    for unit in obs.observation.raw_units:
      if unit.is_selected:
        unit_info = f'unit {hex(unit.tag)}({str(units.get_unit_type(unit.unit_type))}) {unit.order_id_0} {unit.x, unit.y}'
        print(f"selected unit (raw): {unit_info}")
    for unit in obs.observation.feature_units:
      if unit.is_selected:
        unit_info = f'unit {hex(unit.tag)}({str(units.get_unit_type(unit.unit_type))}) {unit.order_id_0} {unit.x / 256, unit.y / 256}'
        print(f"selected unit (fea): {unit_info}")
